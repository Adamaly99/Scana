...
async function decryptBytes(record: Pick<MetaRecord, "iv" | "ciphertext">): Promise<ArrayBuffer> {
...
export async function saveLocalOcrResult(result: Omit<LocalOcrResult, "id">): Promise<void> {
  const db = await getDatabase();
