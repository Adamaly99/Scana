...
interface ScanStore {
  ...
  addPage: (input: { dataUrl: string; filter: FilterType; width: number; height: number }) => Promise<string>;
  ...
  persist(
    (set, get) => ({
      ...
      addPage: async (input) => {
        ...
        set((state) => ({
          pages: [...state.pages, { id, filter: input.filter, width: input.width, height: input.height, createdAt: Date.now() }],
        }));
        return id;
      },
      ...
      setFilter: (id, filter) => {
        set((state) => ({
          pages: state.pages.map((page) => (page.id === id ? { ...page, filter } : page)),
        }));
      },
      ...
      setPageOrder: (orderedIds) => {
        set((state) => {
          if (orderedIds.length !== state.pages.length) return state;
          const byId = new Map(state.pages.map((page) => [page.id, page]));
          const next = orderedIds.map((id) => byId.get(id)).filter((page): page is ScannedPage => Boolean(page));
      ...
      saveCurrentAsDocument: (name) => {
        ...
        const id = makeId("doc");
        const document: ScanDocument = { id, name: name.trim() || "Document sans titre", pages: [...pages], createdAt: Date.now() };
        set((state) => ({ documents: [document, ...state.documents], pages: [] }));
        return id;
      },
      ...
      deleteDocument: async (id) => {
        ...
        if (document) {
          await deleteImageBlobs(document.pages.map((page) => page.id));
        }
      ...
      renameDocument: (id, name) => {
        ...
        set((state) => ({
          documents: state.documents.map((document) => (document.id === id ? { ...document, name: cleanedName || document.name } : document)),
        }));
      },
      ...
      setDocumentPageFilter: (documentId, pageId, filter) => {
        set((state) => ({
          documents: state.documents.map((document) => {
            ...
            return { ...document, pages: document.pages.map((page) => (page.id === pageId ? { ...page, filter } : page)) };
          }),
        }));
      },
      ...
      rotateDocumentPage: async (documentId, pageId, direction) => {
        ...
        const rotated = await rotateImageBlob90(blob, direction);
        await saveImageBlob(page.id, rotated.blob);
        set((state) => ({
          documents: state.documents.map((item) => {
            if (item.id !== documentId) return item;
            return { ...item, pages: item.pages.map((currentPage) => (currentPage.id === pageId ? { ...currentPage, width: rotated.width, height: rotated.height } : currentPage)) };
          }),
        }));
      },
